...
class ThemeManager(QObject):
    ...
    def get_stylesheet(self) -> str:
        """
        Get the complete light-mode stylesheet with current LightTheme colors.

        The stylesheet template uses the DEFAULT color constants (#2563eb etc.).
        On every regeneration we replace those defaults with whatever LightTheme
        currently holds, so theme updates from extensions are reflected instantly.
        """
        if self._stylesheet_cache is not None:
            return self._stylesheet_cache

        # Defaults that ship in LIGHT_STYLESHEET
        _DEFAULT_PRIMARY = "#2563eb"
        _DEFAULT_PRIMARY_DARK = "#1d4ed8"
        _DEFAULT_PRIMARY_LIGHT = "#dbeafe"

        stylesheet = LIGHT_STYLESHEET

        # Only substitute if the color has actually been changed from defaults
        if LightTheme.PRIMARY != _DEFAULT_PRIMARY:
            stylesheet = stylesheet.replace(
                _DEFAULT_PRIMARY_DARK, LightTheme.PRIMARY_DARK
            )
            stylesheet = stylesheet.replace(
                _DEFAULT_PRIMARY_LIGHT, LightTheme.PRIMARY_LIGHT
            )
            # Replace PRIMARY last (it's a substring of PRIMARY_DARK / PRIMARY_LIGHT urls)
            stylesheet = stylesheet.replace(_DEFAULT_PRIMARY, LightTheme.PRIMARY)
